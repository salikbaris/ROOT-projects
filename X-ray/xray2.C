...
tuple<Double_t, Double_t> FindPeakEnds(TGraph *gr, Double_t peak_idx, Int_t start_point = 2, Int_t points_searched = 30, Double_t tolerance = 5) {
...
void xray2() {
    ...
    ////////////////////////////////////////////////
    /// Getting the experimental angles
    ...
    for (Int_t i = 0; i < num_peaks; i++) {
        ...
        // For the first peak, couldn't find a better way to correctly set the range.
        if (i == 0) fit_x2 = 7.5;
        ...
        // The last peak is a special case, as it comes without beta1.
        if (i == num_peaks - 1) {
            n += 1;
    ...
    // Disabling the fit box. Enabling it further down affects this.
    ...
    ////////////////////////////////////////////////
    /// Calculating the theoretical angles
    ...
    for (Int_t i = 0; i < k_alpha1_lines_exp.size(); i++) {
        
        // Checking if we still have beta1 angles.
        if (i < k_beta1_lines_exp.size()) {
        ...
        // ey is not used. Some hard-coded variables may need to be changed if used.
        AddPointWithErrors(gr_calib_fit, x, y, ex, 0);
        
        // Outputting the expected and found angles to the terminal.
        Printf("found: %.2f +- %.2f, expected: %f", x, ex, y);
        }
    ...
    while ((entry = gSystem->GetDirEntry(dirp))) {
        ...
        // .txt files should be exclusively data files.
            ...
            c_analyze->Print(Form("%d kV.pdf", (Int_t)(V/1000)));
            ...
            Printf("V: %.f, lambda_min: %.2g +- %.2g, f_max: %.2g +- %.2g", V, lambda_min, lambda_min_err, f_max, f_max_err);
